...
class ProtoNet(nn.Module):
    def __init__(self, args):
        ...
        self.memory_bank = MemoryBank(args.memory_bank_size)
    ...
    def forward(self, x, memory_bank = None, get_feature = False):
        if get_feature:
            ...
            for cycle in range(self.args.cycles + 1):
                ...
                debug_support = x[support_idx.flatten()].view(*(support_idx.shape + (-1,)))
                support = instance_embs[support_idx.flatten()].view(*(support_idx.shape + (-1,)))
                query = instance_embs[query_idx.flatten()].view(  *(query_idx.shape   + (-1,)))

                logits = self._forward(support, query, memory_bank = True, debug_support = debug_support)
